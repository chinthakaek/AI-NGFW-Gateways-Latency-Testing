#This application enables to use multiple-ENIs to conduct tests without requiring firewall commits
...
st.sidebar.markdown("---")
st.sidebar.subheader("Multi-IP / ENI Binding Settings")
ip_uc1_uc5 = st.sidebar.text_input("UC1 & UC5 Source IP (ens5)", value="10.30.1.82", help="No Decryption, No Profiles")
ip_uc2     = st.sidebar.text_input("UC2 Source IP (ens6)",     value="10.30.1.83", help="Decryption ON, No Profiles")
ip_uc3_uc4 = st.sidebar.text_input("UC3 & UC4 Source IP (ens7)", value="10.30.1.84", help="Decryption ON, AI Profiles ON")
...
# ==========================================
# Custom HTTP Adapter for Source IP Binding (Requests - UC5)
# ==========================================
...
# ==========================================
def get_bedrock_client(cold_conn, source_ip):
    """
    Initializes a Boto3 client and strictly binds its internal connection pool 
    manager to the specified local ENI source IP.
    """
    if cold_conn:
        config = Config(
    ...
    # Safely inject the source IP binding directly into botocore's internal urllib3 pool
    try:
        pool_manager = client._endpoint.http_session._manager
        if pool_manager.connection_pool_kw is None:
            pool_manager.connection_pool_kw = {}
        pool_manager.connection_pool_kw['source_address'] = (source_ip, 0)
    except Exception as e:
        st.error(f"Failed to bind boto3 connection to source IP {source_ip}: {e}")
...
def execute_airs_sync_request(scenario, prompt, api_key, profile_name, region_url, source_ip):
    ...
    # Session configured with SourceIPAdapter to bind to the specific ENI
...
with tab1:
    ...
    # Automatically map the scenario to the assigned Source IP
    if scenario_id in ["UC1", "UC5"]:
        target_source_ip = ip_uc1_uc5
    elif scenario_id == "UC2":
        target_source_ip = ip_uc2
    else:  # UC3 & UC4
        target_source_ip = ip_uc3_uc4

    st.info(f"ℹ️ **AUTOMATED ENI ROUTING:** Traffic for **{scenario_id}** will originate from **`{target_source_ip}`**. No firewall config commit is needed between runs.")
    ...
    if (scenario_id in ["UC1", "UC2", "UC3", "UC5"] and not benign_prompts_list) or (scenario_id == "UC4" and not jailbreak_prompts_list):
        ...
        if start_run_click:
            if scenario_id == "UC5" and (not airs_api_key or not airs_profile_name):
                ...
                if not cold_connection and scenario_id != "UC5": 
                    client = get_bedrock_client(False, target_source_ip)
                    ...
                    for idx in range(1, num_requests + 1):
                        ...
                        if scenario_id == "UC5":
                            result = execute_airs_sync_request(
                                scenario_id, current_prompt, airs_api_key, airs_profile_name, airs_region, target_source_ip
                            )
                        else:
                            if cold_connection: 
                                client = get_bedrock_client(True, target_source_ip)
                            
                            result = execute_single_request(
                                client, scenario_id, current_prompt, max_tokens_val, 
